...
import { Building2, Inbox, Users, AlertCircle } from "lucide-react";
import { createClient } from "@/lib/supabase/server";

export default async function AdminDashboard() {
  const supabase = createClient();

  const [propsRes, availRes, inqRes, openInqRes, subsRes, confSubsRes] = await Promise.all([
  ...
  const stats = [
    ...
    { label: "Total subscribers",      value: subsRes.count ?? 0,     icon: Users, href: "/admin/subscribers" },
  ];
