  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl sm:text-3xl font-serif font-semibold">Receipts</h1>
              ...
              <tbody className="divide-y">
                {receipts.map((r) => (
                  <tr key={r.id} className="hover:bg-secondary/20 transition-colors">
                    <td className="px-4 py-3 font-mono font-semibold text-primary whitespace-nowrap">{r.receipt_number}</td>
                    <td className="px-4 py-3">
                      <div className="font-medium whitespace-nowrap">{r.buyer_name}</div>
                      <div className="text-xs text-muted-foreground">{r.buyer_email}</div>
                    </td>
                    <td className="px-4 py-3 text-muted-foreground max-w-[160px]">
                      {r.properties ? (
                        <span className="line-clamp-2">{r.properties.title}<br /><span className="text-xs">{r.properties.city}, {r.properties.state}</span></span>
                      ) : "—"}
                    </td>
                    <td className="px-4 py-3 font-semibold whitespace-nowrap">{fmt(r.amount_ngn)}</td>
                    <td className="px-4 py-3 text-muted-foreground capitalize whitespace-nowrap">{r.payment_method.replace(/_/g, " ")}</td>
